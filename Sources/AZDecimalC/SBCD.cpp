...
//
// 【アルゴリズム選択の理由】
// 各桁の商（0〜9）を「除数を引ける回数を数える」線形探索で求めている。
// 先頭桁推定法（Trial Quotient Estimation）を使えば内部ループを最大10回から
// 最大3回程度に削減できるが、採用しなかった理由は以下の通り：
//
//   1. 正確性：線形探索は常に正確な商を返し、推定誤差の補正ロジックが不要
//   2. シンプルさ：コードが短く、レビュー・検証が容易
//   3. 性能：最悪ケース (SBCD_PRECISION=60) でも
//            60桁 × 10回 × 60演算 = 36,000 char演算 ≒ マイクロ秒オーダー
//            電卓アプリの用途では実用上十分な速度である
//---------------------------------------------------------------------------
